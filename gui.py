...
def main():
    ...
    circle_cost_field_calculation(obstacles, field)
        ...
        for i in range(-1, 2):
            for j in range(-1, 2):
                if (i != 0 or j != 0) and (0 < y+i < FIELD_DIM_Y and 0 < x+j < FIELD_DIM_X) and not [x+j, y+i] in path:
                    cost = field[y+i][x+j] + (-GOAL_BIAS * j * unitWith0(end[0]-x)) + (-GOAL_BIAS * i * unitWith0(end[1]-y)) # TODO for tuesday: goal bias here could also be supplemented by stepth???
...
def smooth(path, field):
    ...
    for i in range(1, len(path)):
        ...
        smooth_cost = direct_cost(field, start_coord, cur_coord)
...
def direct_cost(field, start, end):
    x = start[0]
    y = start[1]
    cost = 0
    path = [start]
...
def circle_cost_field_calculation(obstacles, field):
